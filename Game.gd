...
onready var camera = get_node("Player/CameraContainer/Camera")
...
#		target.transform.origin = target_point
	...
	if event is InputEventMouseMotion:
		if event.button_mask & (BUTTON_MASK_MIDDLE + BUTTON_MASK_LEFT):
			camrot += event.relative.x * -0.005
			get_node("Player/CameraContainer").set_rotation(Vector3(0, camrot, 0))
